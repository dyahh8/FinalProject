...
def main():
    ...
    try:
        ...
        user_info = db.users_wisata.find_one({"email": payload["id"]})
...
def sign_up():
    ...
    db.users_wisata.insert_one(doc)
...
def sign_in():
    ...
    print(pw_hash)
    result = db.users_wisata.find_one(
        {
            "email": email_receive,
            "password": pw_hash,
        }
...
def add_wisata():
    ...
    db.wisata_international.insert_one({
        'name': name,
...
def get_wisata():
    wisata = db.wisata_international.find()
    wisata_list = []
...
def edit_wisata(id):
    ...
    existing_wisata = db.wisata_international.find_one({'_id': ObjectId(id)})
    ...
    db.wisata_international.update_one(
        {'_id': ObjectId(id)},
        {
            '$set': {
                'name': name,
...
def delete_wisata(post_id):
    existing_wisata = db.wisata_international.find_one({'_id': ObjectId(post_id)})
    existing_file_path = existing_wisata['image_wisata']
    os.remove(existing_file_path)
    result = db.wisata_international.delete_one({'_id': ObjectId(post_id)})
...
def get_wisata_detail(wisata_id):
    attraction = db.wisata_international.find_one({'_id': ObjectId(wisata_id)})
    token_receive = request.cookies.get(TOKEN_KEY)
    logged_in = False
    try:
        payload = jwt.decode(
...
def book_ticket():
    ...
    attraction = db.wisata_international.find_one({'_id': ObjectId(attraction_id)})
    ...
    db.wisata.update_one({'_id': ObjectId(attraction_id)}, {
                         '$set': {'total_tickets': updated_tickets}})
...
def edit_proof(booking_id):
    ...
    if proof_file:
        ...
        filename = f'static/bukti_bayar-{nama}-{wisata}.{extension}'
        proof_file.save(filename)
    else:
        filename = existing_book.get('proof')
    print("Updated Proof File:", filename)
    db.bookings.update_one({'_id': ObjectId(booking_id)}, {
                           '$set': {'proof': filename}})
    return jsonify({'message': 'Bukti sudah diupload'}), 200
...
def generate_pdf(booking_id):
    ...
    pdf.drawString(
        100, 500, f"Dibayar lunas sejumlah {specific_card['total_price']}")
    ...
    response.headers.set('Content-Disposition', 'attachment',
                         filename=f"{specific_card['name']} Tiket {specific_card['wisata']}.pdf")
...
def domestic():
    ...
    try:
        ...
        is_admin = user_info.get("category") == "admin"
        logged_in = True
        wisata_domestic = db.wisata.find()
        wisata_list = []

        for attraction in wisata_domestic:
            wisata_list.append({
                'id': str(attraction['_id']),
                'name': attraction['name'],
                'description': attraction['description'],
        ...
        return render_template('domestic.html', user_info=user_info, logged_in=logged_in, is_admin=is_admin, wisata_list=wisata_list)
    except jwt.ExpiredSignatureError:
        msg = 'Token Anda telah kadaluarsa'
    except jwt.exceptions.DecodeError:
        msg = 'Ada masalah saat login'
    return render_template('domestic.html', msg=msg)
...
def international():
    ...
    try:
        ...
        is_admin = user_info.get("category") == "admin"
        logged_in = True
        wisata_international = db.wisata_international.find()
        wisata_list = []

        for attraction in wisata_international:
            wisata_list.append({
                'id': str(attraction['_id']),
                'name': attraction['name'],
                'description': attraction['description'],
        ...
        return render_template('international.html', user_info=user_info, logged_in=logged_in, is_admin=is_admin, wisata_list=wisata_list)
    except jwt.ExpiredSignatureError:
        msg = 'Token Anda telah kadaluarsa'
    except jwt.exceptions.DecodeError:
        msg = 'Ada masalah saat login'
    return render_template('international.html', msg=msg)
